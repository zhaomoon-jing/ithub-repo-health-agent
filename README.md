...
```bash
pytest          # 32 个测试全过，含 LLM 相关用例（mock 不消耗真实 token）
```
...
详见 **`我的学习.md`**（项目内同名文件）——这是一份面向"非作者也能讲清项目"的超详细讲解文档，含名词解释、每个函数说明、问答脚本与踩坑记录。
...
4. **NLU 能力落地**：D8 的追问 Agent 显式做意图识别（有限意图分类）+ 上下文追踪，贴合自然语言理解（NLU）场景。
...
- **Issue / PR 计数改用 Search API（2026-08）**：GitHub 正将 `/issues` 端点从 offset 分页迁移至 cursor 分页，实测该端点已不再返回 `rel="last"`，导致旧实现按 `Link` header 分页计数时"开放 Issue 数"失真（因 `per_page=1` 回退为 1）。现四个指标——开放 Issue、近 30 天关闭 Issue、开放 PR、近 30 天合并 PR——统一走 `/search/issues` 的 `total_count`，并用 `is:issue` / `is:pr` 精确区分，既准确又免疫 cursor 化。实测对照（psf/requests）：修复前 1 / 86（失真）→ 修复后 148 / 86（合理）。
